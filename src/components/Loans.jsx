// src/components/Loans.jsx
import React from 'react';

const Loans = () => {
  return (
    <section className="bg-white p-8">
      <div className="container mx-auto">
        <h2 className="text-2xl font-bold mb-4">Loans</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {renderLoan('MSME')}
          {renderLoan('Collateral Free')}
          {renderLoan('Housing Loan')}
          {renderLoan('Subsidy Loan Overdraft')}
          {renderLoan('CC Limit')}
          {renderLoan('Loan Against Property')}
          {renderLoan('Project Report')}
        </div>
      </div>
    </section>
  );
};

const renderLoan = (title, subtitle) => (
  <div className="bg-gray-100 p-4 rounded-lg shadow-md transition duration-300 ease-in-out transform hover:scale-105">
    <h3 className="text-lg font-semibold mb-2">{title}</h3>
    <p>{subtitle}</p>
  </div>
);

export default Loans;
